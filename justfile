...
compile in_dir out_dir='':
	...
	def compile_idl(p: str, includes: list, package_name: str | None = None, out_path: Path | None = None) -> str:
		...
		o = HashTypeDescription('').generate_type_hashes(
			package_name=package_name,
			interface_files=[f"{path.parent}:{path.name}"], # Legacy code hack; the change will be restored
			include_paths=includes,
			output_path=out_path
		)[0]
		return str(o) # rosidl bug: generate_type_hashes claims to return List[str] but actually returns List[Path]
	...
	if __name__ == '__main__':
		...
		my_in = Path(r"{{ in_dir }}")
		my_out_dir: Path | None = $out_dir_expr
		...
		for idl in my_in_glob:
			...
			if idl.parent.name not in ACCEPTED_IDL_INNER_MODULE:
				...
				moved = Path(shutil.move(idl, d))
				original_dir_of[moved] = idl.parent
				my_idls.append(str(moved))
				package_name_of[str(moved)] = package_name
			else:
		...
		my_jsons = set(my_jsons)
		for moved in original_dir_of.keys():
			original_dir = original_dir_of[moved]
			shutil.move(moved, original_dir)
			maybe_o = str(moved).rsplit('.', 1)[0] + '.json'
			if maybe_o in my_jsons:
				# If indeed generated by us, move along with the IDL
				o = Path(maybe_o)
				(original_dir / o.name).unlink(missing_ok=True)
				my_jsons.add(str(shutil.move(o, original_dir)))
				my_jsons.remove(maybe_o)
			if not any(moved.parent.iterdir()): moved.parent.rmdir()
		for o in my_jsons:
			if my_out_dir is not None:
				o = Path(o)
